...
<div class="w-[100vw] h-[100vh] bg-[url(/img/imglogin.jpg)] bg-no-repeat bg-cover bg-fixed pt-5 relative">
    ...
    <div class="w-[80%] md:w-[90%] h-[80vh] bg-orange-600 m-auto mt-5 p-8 pt-3 rounded-xl relative shadow-xl overflow-hidden flex flex-col">
      ...
      <div class="mb-2 relative bg-zinc-700 rounded-md w-[100%] p-5 flex items-center gap-4">
        <h1 class="text-2xl md:text-base xl:text-3xl lg:text-lg font-semibold text-zinc-800 bg-orange-600 p-1 px-4 rounded-full">Lista de Reportes</h1>
         ...
         <tbody id="report-list" class="bg-zinc-700 text-orange-600 rounded-md text-center">
            ...
            <tr class="hover:bg-zinc-600 transition-colors duration-200 add-row">
                <td colspan="4" class="py-4">
                    <a href="{{ route('reporte.create') }}" class="flex items-center justify-center space-x-2">
                        ...
                        <span class="text-lg text-orange-600 group-hover:text-orange-500">Crear nuevo reporte</span>
                    </a>
                </td>
            </tr>
            ...
            <tr data-id="{{ $reporte->id }}" data-media-files="{{ json_encode($reporte->media_files) }}" class="hover:bg-zinc-600 transition-colors duration-200">
                ...
                <td class="p-2">
                    ...
                    <div class="flex flex-row justify-center gap-1 mb-1">
                        @foreach($reporte->media_files->take(2) as $media)
                        ...
                        @endforeach
                        @if($reporte->media_files->count() > 2)
                            <div class="relative bg-zinc-700 rounded-lg p-1">
                                <div class="w-16 h-16 bg-zinc-600 rounded-lg flex items-center justify-center">
                                    <span class="text-orange-600 text-sm">+{{ $reporte->media_files->count() - 2 }}</span>
                                </div>
                            </div>
                        @endif
                    </div>
